...
public class KMeansClusteringHandlerImpl {
    private static final Logger log = LogManager.getLogger(KMeansClusteringHandlerImpl.class);

    public static Dataset<Row> buildModel(int k, Dataset<Row> featureDF) {
        // KMeans Clustering
    ...
    public static void buildModelWithPCA(int clusterCount, int principalComponentCount, Dataset<Row> featureDF) {
        ...
        KMeans kMeans1 = new KMeans().setK(clusterCount);
    ...
    public static void writeToStream(Dataset<Row> predictDF, StreamObserver<ModelResponse> responseObserver) {
